...
### 4.3 坐标约定（曾踩坑！）
统一 **0.0~1.0 的相对比例**（小数）：`x = 像素x / 图宽`，`y = 像素y / 图高`。

**小数位数不设上限**（2026-09 放开，原先写死"保留 3~4 位"）：模型若能感知到更细的位置就照实多写，
3 位不够时写 0.35125、0.351253 都合法；0~1000 刻度同样允许带小数（350.25 → 0.35025）。
下游对 0~1 的值**原样透传、不做四舍五入**，只有 0~1000 换算那条路会 `round(..., 6)`。
为什么值得放开：**小目标的相对误差**——只占画面宽度 0.4% 的物件，宽度写成 3 位小数的 0.004
就被放大了 6.7%。实测在 4000px 宽画布上 0.35125 与 0.351 落在**不同像素列**（1405 vs 1404），
而 800px 画布上两者只差 0.2px、根本测不出来（`runs/verify_precision.py`）。
⚠️ 放开位数是**有代价**的：模型很可能"编精度"（末位其实是猜的），所以提示词里必须同时写明
...
### 4.4 系统提示词
`objloc/config.py: DEFAULT_SYSTEM_PROMPT`（可用 `SYSTEM_PROMPT` 覆盖）。当前为显式换算版 + 分辨率说明版 + 精度版，
