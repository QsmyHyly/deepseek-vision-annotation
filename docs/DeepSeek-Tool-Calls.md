...
这里以获取用户当前位置的天气信息为例，展示了使用 Tool Calls 的完整 Python 代码。Tool Calls 的具体 API 格式请参考对话补全文档。
...
- Anthropic API（`/messages`）与 Responses API 支持在对话中间插入工具调用消息，也支持在对话中间插入 system 消息；
