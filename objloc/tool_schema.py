"""把 objloc 里的普通函数转换为可加入模型请求的工具列表对象。

（历史上本模块服务于已删除的 point_parser.py，现在统一由 objloc/tools/registry.py
在注册工具时调用，构建默认工具集见 objloc/tools/builtin.py。）
...
用法：
    from objloc.tool_schema import TOOLS
...
from objloc.parsing import decode_json_points, extract_coordinates, parse_coordinates
